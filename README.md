...
6. 如果使用蓝牙信号条件，按系统提示授予蓝牙权限。
7. 当当天第一次满足条件时，确认已经完成实际打卡。
...
| 蓝牙信号 | 设备、最低 RSSI | 设备最近 30 秒可见且平滑后的信号达到阈值时满足 |
...
1. 点击“扫描”并允许 Daka 访问蓝牙。
2. 从列表选择附近设备。
3. 设置最低信号值，例如 `-65 dBm`。数值越接近 `0`，通常表示距离越近。
4. 保存设置后，设备最近 30 秒内可见且 5 个有效样本的中位数达到阈值时，
   条件才会满足。

扫描结果会明确区分以下状态：

- 未发现附近设备：蓝牙可用，但扫描期间没有收到设备广播。
- 蓝牙已关闭：点击“打开蓝牙设置”启用蓝牙后重新扫描。
- 蓝牙权限被拒绝：点击“打开隐私设置”，在“隐私与安全性 → 蓝牙”中允许
  Daka 访问。
- 当前 Mac 不支持蓝牙扫描：该设备不能使用蓝牙信号条件。

暂停自动统计后，Daka 会停止用于规则判断的持续蓝牙扫描；在设置页手动点击
“扫描”仍会执行一次短时设备发现。
...
5. 当前 SSID 是否与配置完全一致。
6. 使用蓝牙条件时，检查扫描页是否提示蓝牙关闭或权限被拒绝。
7. 确认目标设备正在广播，且 RSSI 达到配置的最低信号值。
